...
# ---------------------------------------------------------------------------
# Core student / subject / program models
# ---------------------------------------------------------------------------
...
class Student(Base):
    ...
    exams = relationship(
        "Exam", back_populates="student", cascade="all, delete-orphan"
    )
    ...
    task_assignments = relationship(
        "TaskAssignment", back_populates="student", cascade="all, delete-orphan"
    )
...
class ProgramSubject(Base):
    ...
    __table_args__ = (
        UniqueConstraint("program_id", "subject_code", name="uq_program_subject"),
        CheckConstraint(
            "semester >= 0 AND semester <= 8", name="ck_program_subject_semester"
        ),
        CheckConstraint(
    ...
    subject_code: Mapped[str] = mapped_column(ForeignKey("subjects.code"), nullable=False)

    semester: Mapped[int] = mapped_column(Integer, nullable=False)
    is_mandatory: Mapped[bool] = mapped_column(Boolean, nullable=False)
    elective_group_code: Mapped[str | None] = mapped_column(String(20), nullable=True)
...
class ProgramSemesterRule(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "semester >= 1 AND semester <= 8", name="ck_program_semester_rule_semester"
        ),
        CheckConstraint("slots > 0", name="ck_program_semester_rule_slots"),
        Index(
            "ix_program_semester_rule_program_semester", "program_id", "semester"
        ),
    )
    ...
    semester: Mapped[int] = mapped_column(Integer, nullable=False)
    elective_group_code: Mapped[str] = mapped_column(String(20), nullable=False)
...
class SubjectRequirement(Base):
    ...
    min_passed_subjects: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class Enrollment(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "student_index", "subject_code", "semester", name="uq_enrollment"
        ),
        CheckConstraint(
            "semester >= 1 AND semester <= 8", name="ck_enrollment_semester"
        ),
    ...
    semester: Mapped[int] = mapped_column(Integer, nullable=False)
    listened: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
...
class ExamSession(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "subject_code", "session_type", "year", name="uq_exam_session"
        ),
    ...
    exams = relationship(
        "Exam", back_populates="exam_session", cascade="all, delete-orphan"
    )
...
class Exam(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "exam_session_id", "student_index", name="uq_exam_student_session"
        ),
...
class ConsultationBooking(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "professor_id", "date", "start_time", name="uq_professor_date_start"
        ),
    )
...
class Task(Base):
    """Professor-created task (assignment template)."""

    __tablename__ = "tasks"
    __table_args__ = (Index("ix_tasks_subject", "subject_id"),)
    ...
    created_by_professor_id: Mapped[int | None] = mapped_column(
        ForeignKey("professors.id"), nullable=True
    )
...
class TaskAssignment(Base):
    """Assignment of a Task to one student (one row per student)."""

    __tablename__ = "task_assignments"
    __table_args__ = (
        UniqueConstraint(
            "task_id", "student_index", name="uq_task_assignment_student"
        ),
        Index("ix_task_assignments_task", "task_id"),
        Index("ix_task_assignments_student", "student_index"),
    )
    ...
    linked_repo_owner: Mapped[str | None] = mapped_column(
        String(255), nullable=True
    )
    linked_repo_name: Mapped[str | None] = mapped_column(
        String(255), nullable=True
    )
    linked_repo_url: Mapped[str | None] = mapped_column(
...
class TaskSubmission(Base):
    """Snapshot of a submission: repo + commit SHA at submit time."""

    __tablename__ = "task_submissions"
    __table_args__ = (Index("ix_task_submissions_assignment", "task_assignment_id"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    task_assignment_id: Mapped[int] = mapped_column(
        ForeignKey("task_assignments.id"), nullable=False
    )
    ...
    task_assignment = relationship("TaskAssignment", back_populates="submissions")


class GitHubAccount(Base):
    """One GitHub account per user (student links for submissions)."""

    __tablename__ = "github_accounts"
    __table_args__ = (UniqueConstraint("user_id", name="uq_github_user"),)
